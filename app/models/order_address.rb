class OrderAddress
  ...
  attr_accessor :post_number, :city, :address, 
                :phone_number, :building, :prefecture_id, :order_id, :item_id, :user_id, :hoge

  with_options presence: true do
    validates :post_number, format: { with: /\A\d{3}[-]\d{4}\z/, message: 'Input correctly' }
    validates :city
    validates :address
    validates :phone_number, numericality: { only_interger: true, message: 'Half-width number' }, length: { maximum: 11, message: 'Over number'}
  end
